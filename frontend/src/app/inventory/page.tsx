            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[680px] text-sm">
                <thead className="border-b border-line text-left text-xs uppercase tracking-normal text-gray-500">
                  <tr><th className="py-2">Product</th><th>On Hand</th><th>Reorder</th><th>Batches</th><th>Status</th></tr>
                </thead>
                <tbody>
                  {(inventory.data ?? []).map((row) => (
                    <tr key={row.product_id} className="border-b border-line last:border-0">
                      <td className="py-3"><div className="font-medium">{row.product_name}</div><div className="text-xs text-gray-500">{row.sku}</div></td>
                      <td>{row.on_hand}</td>
                      <td>{row.reorder_level}</td>
                      <td>{row.batches.map((batch) => `${batch.batch_number}: ${batch.quantity_on_hand}`).join(", ") || "-"}</td>
