...
export default function PurchasesPage() {
  ...
  const createPo = useMutation({
    ...
    onError: (error: any) => toast.error(error.response?.data?.detail ?? "Unable to create purchase order")
  });
  ...
  const receive = useMutation({
    ...
    onError: (error: any) => toast.error(error.response?.data?.detail ?? "Unable to receive goods")
  });
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[700px] text-sm">
                <thead className="border-b border-line text-left text-xs uppercase tracking-normal text-gray-500">
                  <tr><th className="py-2">PO</th><th>Supplier</th><th>Status</th><th>Total</th><th>Created</th></tr>
                </thead>
